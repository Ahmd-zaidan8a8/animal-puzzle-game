        ...
        Road.allRoads.forEach((road) => {
          if(this !== road ) {
            ...
            if(intersectionPoint ) 
              this.storedIntersectionPoint(intersectionPoint)
          }
        });
...
class RoadElement {
  ...
  handlekey(event) {
    if (this.isSlected()) {
      ...
      if (event.key === "ArrowRight" && this.road.isHorizontal()) {
        if (currentLeft <= maxX)
          this.element.style.left = `${currentLeft + 10}px`;
      }
      if (event.key === "ArrowLeft" && this.road.isHorizontal()) {
        if (currentLeft >= minX)
          this.element.style.left = `${currentLeft - 10}px`;
      }
      if (event.key === "ArrowUp" && this.road.isVertical()) {
        if (currentTop >= minY) this.element.style.top = `${currentTop - 10}px`;
      }
      if (event.key === "ArrowDown" && this.road.isVertical()) {
        if (currentTop <= maxY) this.element.style.top = `${currentTop + 10}px`;
      }
    }
  }
  handleIntersection(){
...
console.log(road4.getIntersectionsPoints());
console.log(road1.getIntersectionsPoints());
console.log(road2.getIntersectionsPoints());